...
def start() -> None:
    ...
    frames_tuple = load_video_in_memoery(roop.globals.target_path)
    # initalize global variable
    roop.globals.output_frames = [None] * len(frames_tuple)
    # process frame
    if frames_tuple:
        for frame_processor in get_frame_processors_modules(roop.globals.frame_processors):
            update_status('Progressing...', frame_processor.NAME)
            frame_processor.process_video(roop.globals.source_path, frames_tuple)
            frame_processor.post_process()
    else:
    ...
    create_video_from_frames(roop.globals.output_frames, fps)
    # # handle audio
...
def create_video_from_frames(frames, fps):
    ...
    output_path = roop.globals.target_path + 'new.mp4'
...
def load_video_in_memoery(video_path):
    ...
    cap.release()
    print(f"Total frames in tuple {len(frames_tuple)}")
    return frames_tuple
